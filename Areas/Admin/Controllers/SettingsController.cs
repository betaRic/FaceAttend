    ...
    public class SettingsController : Controller
    {
        ...
        public ActionResult Save(SettingsVm vm)
        {
            ...
            using (var db = new FaceAttendDBEntities())
            {
                ...
                // Save as invariant strings
                SystemConfigService.Upsert(db, "Biometrics:DlibTolerance", vm.DlibTolerance.ToString(CultureInfo.InvariantCulture), "double",
                    "Dlib face distance tolerance. Lower = stricter match.", by);

                SystemConfigService.Upsert(db, "Biometrics:LivenessThreshold", vm.LivenessThreshold.ToString(CultureInfo.InvariantCulture), "double",
                    "Minimum liveness score to accept a scan.", by);

                SystemConfigService.Upsert(db, "Location:GPSAccuracyRequired", vm.GPSAccuracyRequired.ToString(CultureInfo.InvariantCulture), "int",
                    "Max GPS accuracy (meters). Higher means less strict.", by);

                SystemConfigService.Upsert(db, "Location:GPSRadiusDefault", vm.GPSRadiusDefault.ToString(CultureInfo.InvariantCulture), "int",
                    "Default office geofence radius (meters) when office radius is not set.", by);

                SystemConfigService.Upsert(db, "Kiosk:FallbackOfficeId", vm.FallbackOfficeId.ToString(CultureInfo.InvariantCulture), "int",
                    "Office used for desktop kiosks when GPS is skipped. 0 = first active office.", by);

                SystemConfigService.Upsert(db, "Attendance:MinGapSeconds", vm.MinGapSeconds.ToString(CultureInfo.InvariantCulture), "int",
                    "Minimum seconds between scans to prevent double taps.", by);

                SystemConfigService.Upsert(db, "NeedsReview:NearMatchRatio", vm.NeedsReviewNearMatchRatio.ToString(CultureInfo.InvariantCulture), "double",
                    "If distance is within this ratio of the threshold, mark record as NeedsReview.", by);

                SystemConfigService.Upsert(db, "NeedsReview:LivenessMargin", vm.NeedsReviewLivenessMargin.ToString(CultureInfo.InvariantCulture), "double",
                    "If liveness is within this margin above threshold, mark record as NeedsReview.", by);

                SystemConfigService.Upsert(db, "NeedsReview:GPSAccuracyMargin", vm.NeedsReviewGpsMargin.ToString(CultureInfo.InvariantCulture), "int",
                ...
                // Advanced ONNX settings
                SystemConfigService.Upsert(db, "Biometrics:LivenessInputSize", vm.LivenessInputSize.ToString(CultureInfo.InvariantCulture), "int",
                    "Liveness model input size (pixels). Must match the model.", by);

                SystemConfigService.Upsert(db, "Biometrics:Liveness:CropScale", vm.LivenessCropScale.ToString(CultureInfo.InvariantCulture), "double",
                    "Crop scale around detected face before liveness inference.", by);

                SystemConfigService.Upsert(db, "Biometrics:Liveness:RealIndex", vm.LivenessRealIndex.ToString(CultureInfo.InvariantCulture), "int",
                ...
                SystemConfigService.Upsert(db, "Biometrics:Liveness:RunTimeoutMs", vm.LivenessRunTimeoutMs.ToString(CultureInfo.InvariantCulture), "int",
                    "Max ONNX run time before the circuit breaker trips.", by);

                SystemConfigService.Upsert(db, "Biometrics:Liveness:SlowMs", vm.LivenessSlowMs.ToString(CultureInfo.InvariantCulture), "int",
                    "Milliseconds considered 'slow' for liveness inference.", by);

                SystemConfigService.Upsert(db, "Biometrics:Liveness:GateWaitMs", vm.LivenessGateWaitMs.ToString(CultureInfo.InvariantCulture), "int",
                    "Max wait time to enter the single-flight liveness gate.", by);

                SystemConfigService.Upsert(db, "Biometrics:Liveness:CircuitFailStreak", vm.LivenessCircuitFailStreak.ToString(CultureInfo.InvariantCulture), "int",
                    "How many failures before circuit opens.", by);

                SystemConfigService.Upsert(db, "Biometrics:Liveness:CircuitDisableSeconds", vm.LivenessCircuitDisableSeconds.ToString(CultureInfo.InvariantCulture), "int",
        ...
        private SettingsVm BuildVm(FaceAttendDBEntities db)
        {
            ...
            var decision = SystemConfigService.GetString(db, "Biometrics:Liveness:Decision", AppSettings.GetString("Biometrics:Liveness:Decision", "max"));
            var scales = SystemConfigService.GetString(db, "Biometrics:Liveness:MultiCropScales", AppSettings.GetString("Biometrics:Liveness:MultiCropScales", ""));

            var inputSize = SystemConfigService.GetInt(db, "Biometrics:LivenessInputSize", AppSettings.GetInt("Biometrics:LivenessInputSize", 128));
            var cropScale = SystemConfigService.GetDouble(db, "Biometrics:Liveness:CropScale", AppSettings.GetDouble("Biometrics:Liveness:CropScale", 2.7));
            var realIndex = SystemConfigService.GetInt(db, "Biometrics:Liveness:RealIndex", AppSettings.GetInt("Biometrics:Liveness:RealIndex", 1));
            var outputType = SystemConfigService.GetString(db, "Biometrics:Liveness:OutputType", AppSettings.GetString("Biometrics:Liveness:OutputType", "logits"));
            var normalize = SystemConfigService.GetString(db, "Biometrics:Liveness:Normalize", AppSettings.GetString("Biometrics:Liveness:Normalize", "0_1"));
            var chanOrder = SystemConfigService.GetString(db, "Biometrics:Liveness:ChannelOrder", AppSettings.GetString("Biometrics:Liveness:ChannelOrder", "RGB"));
            var timeoutMs = SystemConfigService.GetInt(db, "Biometrics:Liveness:RunTimeoutMs", AppSettings.GetInt("Biometrics:Liveness:RunTimeoutMs", 1500));
            var slowMs = SystemConfigService.GetInt(db, "Biometrics:Liveness:SlowMs", AppSettings.GetInt("Biometrics:Liveness:SlowMs", 1200));
            var gateWaitMs = SystemConfigService.GetInt(db, "Biometrics:Liveness:GateWaitMs", AppSettings.GetInt("Biometrics:Liveness:GateWaitMs", 300));
            var failStreak = SystemConfigService.GetInt(db, "Biometrics:Liveness:CircuitFailStreak", AppSettings.GetInt("Biometrics:Liveness:CircuitFailStreak", 3));
            var disableSec = SystemConfigService.GetInt(db, "Biometrics:Liveness:CircuitDisableSeconds", AppSettings.GetInt("Biometrics:Liveness:CircuitDisableSeconds", 30));
            ...
            // Warn if legacy key exists so you can clean it.
            if (SystemConfigService.HasKey(db, "DlibTolerance") && !SystemConfigService.HasKey(db, "Biometrics:DlibTolerance"))
            {
                vm.WarningMessage = "Legacy key DlibTolerance exists in SystemConfiguration. New key Biometrics:DlibTolerance is preferred.";
            }

            return vm;
        }

        private static System.Collections.Generic.List<SelectListItem> BuildOfficeOptions(FaceAttendDBEntities db, int selected)
        {
            var list = new System.Collections.Generic.List<SelectListItem>
            {
                new SelectListItem { Text = "Auto (first active office)", Value = "0", Selected = selected <= 0 }
            };
