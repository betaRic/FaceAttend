/**
 * FaceAttend - Enrollment Core Module
 * Consolidated enrollment logic shared between admin and mobile enrollment flows
 * 
 * This module provides:
 * - Camera operations (start, stop, capture)
 * - Server communication (scan frame, enroll)
 * - Auto-enrollment loop logic
 * - Frame collection and quality tracking
 * - Image compression utilities
 * - Common UI helpers (SweetAlert modals)
 * 
 * Usage:
...
FaceAttend.Enrollment = (function () {
    ...
    // =========================================================================
    var CONSTANTS = {
        AUTO_INTERVAL_MS: 250,      // Faster scanning (was 350)
        PASS_WINDOW: 3,             // Smaller window (was 5)
        PASS_REQUIRED: 1,           // Capture on first good pass (was 2)
        CAPTURE_WIDTH: 480,
        CAPTURE_HEIGHT: 360,
        UPLOAD_QUALITY: 0.65,
        MAX_IMAGES: 5,
    ...
    /**
     * Pre-check an image file before upload
     */
    ...
    function Enrollment(config) {
        ...
        this.busy = false;
        this.enrolled = false;
        this.enrolling = false;  // Guard for duplicate enrollment
        this.multiFaceWarned = false;
        this.autoTimer = null;
        this.passHist = [];
        this.goodFrames = [];

        // DOM elements (to be set by caller)
        this.elements = {};
    ...
    Enrollment.prototype.stopAutoEnrollment = function () {
        ...
        this.autoTimer = null;
        this.enrolling = false;  // Reset guard when stopped
    };

    Enrollment.prototype.pushGoodFrame = function (blob, probability, encoding) {
        if (!blob) return;
    ...
    Enrollment.prototype.autoTick = function () {
        ...
        this.captureJpegBlob(CONSTANTS.UPLOAD_QUALITY)
            .then(function (blob) {
                capturedBlob = blob;  // Store blob locally
                return self.postScanFrame(blob);
            })
            .then(function (result) {
                // Add the locally captured blob to the result
                if (result) {
    ...
    Enrollment.prototype.processScanResult = function (r) {
        if (!r || r.ok !== true) {
            // Don't treat "no face" or "liveness fail" as serious errors - just status updates
            var errorCode = r && r.error;
            if (errorCode === 'NO_FACE' || errorCode === 'NO_IMAGE') {
                this.handleStatus('No face detected.', 'warning');
                // RESET liveness bar when no face
                ...
                // Serious errors - use handleError which may trigger onEnrollmentError
        ...
        if (pass) {
            this.pushGoodFrame(r.lastBlob || null, p, r.encoding || null);
            ...
            if (this.goodFrames.length < this.config.minGoodFrames) {
            ...
            // Face detected but liveness failed - show current liveness score
            ...
            // Reset pass history on fail to require consecutive passes again
            this.passHist = [];
        }
    };
    ...
    Enrollment.prototype.performEnrollment = function () {
        ...
        // GUARD: Prevent duplicate enrollment calls
        if (this.enrolling) {
            return;
        }
        this.enrolling = true;
        ...
        this.postEnrollMany(frames)
            .then(function (result) {
                self.enrolling = false;  // Reset guard
